  ...
  <div class="py-9">
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
          ...
          <div class="w-full mb-4 flex items-center gap-2 flex-wrap">
              <a href="{{ route('dashboard.equipments.create--page') }}" class="w-fit flex gap-2 items-center p-3 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-500">
              ...
              <a href="{{ route('dashboard.equipments') }}" class="w-fit flex gap-2 items-center p-3 text-sm bg-gray-300 rounded-lg hover:bg-gray-200">
          ...
          <div>
            <form class="flex w-full mt-5 mb-2 rounded bg-white" action="{{ route('dashboard.equipments') }}">
                <input class=" w-full border-none bg-transparent px-4 py-1 text-gray-600 outline-none focus:outline-none " type="search" name="search" placeholder="Buscar..." />
          ...
          @if ($equipments->count() > 0)
            <table class="w-full border-collapse bg-white text-left text-sm text-gray-500">
              ...
              <tr class="hover:bg-gray-50">
                  <th class="flex gap-3 px-6 py-4 font-normal text-gray-900">
                    <div class="relative h-10 w-10">
                      <img
                        ...
                        src="{{ $equipment->image ? asset('storage/images/equipments/'.$equipment->image) : asset('img/not-found.png') }}"
                        alt=""
                      />
                    </div>
                    <div class="text-sm">
                  ...
                  <td class="px-6 py-4">{{ $equipment->category->name }}</td>
                  
                  <td class="px-6 py-4">
                    <div class="flex justify-end gap-4">
                      <form onSubmit="deleteItem(event)" method="post" action="{{ route('dashboard.equipments.delete', ['id' => $equipment->id]) }}">
                          @csrf
                          @method('delete')
                      ...
                      <a x-data="{ tooltip: 'Edite' }" href="{{ route('dashboard.equipments.update--page', ['id' => $equipment->id]) }}">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          fill="none"
                          viewBox="0 0 24 24"
            ...
            <br>
            {{ $equipments->links('components.pagination-tailwind') }}
          @else
            <p class="mt-20 text-center">Ningún equipo medico encontrado. <br> <a class="text-blue-800 underline" href="{{ route('dashboard.equipments') }}">Click aquí</a> para reiniciar filtros</p>
          @endif
